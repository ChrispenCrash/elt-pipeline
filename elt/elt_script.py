...
result = subprocess.run(
    dump_command, env=subprocess_env, capture_output=True, text=True
)
...
result = subprocess.run(
    load_command, env=subprocess_env, capture_output=True, text=True
)

if result.returncode != 0:
    print(f"Error importing pg_dump: {result.stderr}")
